...
CREATE OR REPLACE DYNAMIC TABLE STEAM_REVIEW.BRONZE.steam_reviews_dlq
  TARGET_LAG = '1 hour'
  WAREHOUSE = COMPUTE_WH
  COMMENT = 'Quarantine table for invalid steam_review'
AS
SELECT 
    review_id,
    app_id,
    language,
    review,
    timestamp_created_raw                  AS timestamp_created,
    timestamp_updated_raw                   AS timestamp_updated,
    voted_up_raw                            AS voted_positive,
    votes_up_raw                            AS votes_helpful,
    votes_funny_raw                         AS votes_funny,
    weighted_vote_score_raw                 AS weighted_vote_score,
    comment_count_raw                       AS comment_count,
    steam_purchase_raw                      AS is_steam_purchase,
    received_for_free_raw                   AS is_received_for_free,
    written_during_early_access_raw         AS is_written_during_early_access,
    developer_response,
    timestamp_dev_responded_raw             AS timestamp_dev_responded,
    primarily_steam_deck_raw                AS is_primarily_steam_deck_player,
    reactions_raw                           AS reactions,
    ingested_at,
    REVIEW_FAILURE_REASONS
FROM STEAM_REVIEW.SILVER.steam_reviews_flagged
WHERE ARRAY_SIZE(REVIEW_FAILURE_REASONS) > 0;


CREATE OR REPLACE DYNAMIC TABLE STEAM_REVIEW.BRONZE.steam_reviews_player_info_dlq
  TARGET_LAG = '1 hour'
  WAREHOUSE = COMPUTE_WH
    COMMENT = 'Quarantine table for invalid author/player data. review_id and app_id are kept as keys even though this is player-grain. Without them, you cannot trace a bad author record back to which review row it came from or join back to Bronze/Silver for reprocessing. author_steam_id alone is not a stable join key here since it can itself be NULL.'
AS
SELECT 
    review_id,
    app_id,
    author_steam_id,
    author_num_games_owned_raw              AS author_num_games_owned,
    author_num_reviews_raw                  AS author_num_reviews,
    author_playtime_forever_raw             AS author_playtime_forever,
    author_playtime_last_two_weeks_raw      AS author_playtime_last_two_weeks,
    author_playtime_at_review_raw           AS author_playtime_at_review,
    author_deck_playtime_at_review_raw      AS author_deck_playtime_at_review,
    author_last_played_raw                  AS author_last_played,
    ingested_at,
    REVIEW_PLAYER_FAILURE_REASONS
FROM STEAM_REVIEW.SILVER.steam_reviews_flagged
WHERE ARRAY_SIZE(REVIEW_PLAYER_FAILURE_REASONS) > 0;
