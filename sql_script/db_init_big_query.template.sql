CREATE SCHEMA IF NOT EXISTS `<project-id>.bronze`
OPTIONS(location = "<region>");

CREATE TABLE IF NOT EXISTS `<project-id>.bronze.ingestion_run` (
...
CREATE TABLE IF NOT EXISTS `<project-id>.bronze.raw_review` (
  ...
  run_id STRING NOT NULL,
    ...
    FOREIGN KEY (run_id)
    REFERENCES `<project-id>.bronze.ingestion_run` (run_id) 
    NOT ENFORCED,
...
CREATE SCHEMA IF NOT EXISTS `<project-id>.silver`
OPTIONS(location = "<region>");

-- doc abput field explaination https://partner.steamgames.com/doc/store/getreviews
CREATE TABLE IF NOT EXISTS `<project-id>.silver.clean_latest_review` (
  app_id INT64 NOT NULL,
  review_id INT64 NOT NULL,
...
CREATE TABLE IF NOT EXISTS `<project-id>.silver.clean_latest_review_player_info` (
  app_id INT64 NOT NULL,
  review_id INT64 NOT NULL,
  ...
  CONSTRAINT fk_playerinfo_review
    ...
    REFERENCES `<project-id>.silver.clean_latest_review` (app_id, review_id)
    NOT ENFORCED
)
...
CREATE TABLE IF NOT EXISTS `<project-id>.silver.transform_checkpoint` ( 
  pipeline_name STRING NOT NULL, 
